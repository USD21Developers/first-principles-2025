...
const fpServiceWorker = (() => {
  ...
  const showReloadControl = () => {
    ...
    button.id = "fp-pwa-update"; button.type = "button";
    button.textContent = "New version available — reload";
    Object.assign(button.style, { position: "fixed", left: "50%", bottom: "1rem", zIndex: "2147483647", transform: "translateX(-50%)", padding: ".6rem .9rem", border: "1px solid #fff", borderRadius: ".25rem", background: "#212529", color: "#fff", font: "inherit", boxShadow: "0 2px 8px rgba(0,0,0,.35)", cursor: "pointer" });
    button.addEventListener("click", () => window.location.reload());
    document.body.appendChild(button);
  };
  const watchRegistration = (registration) => registration.addEventListener("updatefound", () => {
    registration.installing?.addEventListener("statechange", (event) => {
      if (event.target.state === "activated" && navigator.serviceWorker.controller) showReloadControl();
    });
  });
  const register = () => {
    if (!registrationPromise) registrationPromise = nativeRegister(workerUrl, { scope: "/fp/", updateViaCache: "none" }).then(async (registration) => {
    watchRegistration(registration);
    const registrations = await container.getRegistrations();
    await Promise.all(registrations.map((item) => item.scope.startsWith(workerScope) && item.scope !== workerScope ? item.unregister() : false));
    await registration.update();
    return registration;
    }).catch((error) => { registrationPromise = null; console.error("Error in registering service worker:", error); throw error; });
  ...
  container.register = (scriptUrl, options) => {
    ...
    return requested.pathname.endsWith("/sw.js") && requested.pathname.startsWith("/fp/") ? register() : originalRegister(scriptUrl, options);
  };
